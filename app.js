// ═══════════════════════════════════════════════════════
//  WavePlayer — app.js  v1.0.6
//  New: Settings menu (theme hidden), remember last song,
//       "What's New" modal, 3-dot context menu per song,
//       Crash fixes: robust error handling, event cleanup,
...
function showWhatsNewModal() {
  const hasSeenV2 = localStorage.getItem('wave_seen_v1_0_6');
  if (hasSeenV2) return;
  ...
  const closeModal = () => {
    ...
    localStorage.setItem('wave_seen_v1_0_6', 'true');
  };
...
document.getElementById('versionInfo').textContent = 'v1.0.6 • June 2026';
...
async function init() {
  ...
  // Show "What's New" modal first time
  showWhatsNewModal();
}
...
// Auto-play next with error handling
